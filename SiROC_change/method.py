...
def obtain_change_map(pre, post, neighborhood, excluded=0):
    ...
    post_pred = pre * post_sum / (pre_sum + 1e-12)
    change_map = torch.abs(post_pred - post)
    change_map = torch.nan_to_num(change_map)
...
def apply_threshold(change_map,j,threshold,l,otsu_factor):
    if threshold == 'Otsu':
        # Use uint8 (0-255) and handle NaNs safely
        j_clean = torch.nan_to_num(j, nan=0.0)
        img_uint8 = np.array(np.abs(j_clean.numpy() * 255), dtype=np.uint8)
        
        t= cv2.threshold(img_uint8,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)[0]
        change_map[l,:,:] = torch.where(torch.abs(j) > (t*otsu_factor/255), torch.tensor(1), torch.tensor(0))
