    ...
    def __init__(self, root, csv_file, transform_op=None):
        ...
        with open(csv_path, 'r') as f:
            ...
            for line in lines:
                ...
                images = os.listdir(os.path.join(root, 'Images', city, 'pair'))
                images.sort()
                images_path = list(map(lambda x: os.path.join('Images', city, 'pair',  x), images))
                pre_img.append(images_path[0])
                post_img.append(images_path[1])

                # Determine label location
                if os.path.exists(os.path.join(root, 'TrainLabels', city)):
                    label_dir = 'TrainLabels'
                elif os.path.exists(os.path.join(root, 'TestLabels', city)):
                    label_dir = 'TestLabels'
                else:
                    raise FileNotFoundError(f"Labels for {city} not found in TrainLabels or TestLabels")

                lab = np.sort(os.listdir(os.path.join(root, label_dir, city, 'cm')))
                lab_path = list(map(lambda x: os.path.join(label_dir, city, 'cm', x), lab))
                lab_path = [x for x in lab_path if 'cm.png' in x] # Remove irrelevant entries
                labels.append(lab_path[0])
                img_list.append(city)
